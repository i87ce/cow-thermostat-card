...
### Fixed
- **XL global setpoint now shows the value when all air zones agree.**
  The bar setpoint button displayed a fixed "—°C"; it now shows the shared
  setpoint when every air zone matches, and "—°C" only when they differ.

## [1.5.2] — 2026-07-03

### Changed — XL scene buttons
- **Scene row repurposed** to whole-house shortcuts: Apri/Chiudi tapparelle
  + Accendi/Spegni luci (interne — escluse 3 esterne, scale, ambilight, router),
  mapped to dedicated scripts (`examples/ha-cow-scenes.yaml`).
- **Scene buttons support `mdi:` icons** via `ha-icon` (previously emoji only),
  with per-button accent colours.
...
### Changed
- **XL home rows restored to full-size tiles (17.5×4 rem).** Luci/tapparelle
  scenes are back to their original dimensions; the row below is a single
  matching line with clima toggle, setpoint (− / value / +), Buongiorno
  and Buonanotte.

## [1.4.24] — 2026-06-13

### Fixed
- **XL action band stacks under the hero.** Luci/tapparelle scenes and the
  clima + Buongiorno/Buonanotte row now live in one flex column directly
  below the hero card (no absolute overlap on the clock/weather area).

## [1.4.23] — 2026-06-13

### Changed
- **XL dashboard fits in 800px height again.** Scene shortcuts are
  shorter; clima + Buongiorno + Buonanotte share one compact four-button
  row; setpoint ± moves inline on the status line when cooling is on.
